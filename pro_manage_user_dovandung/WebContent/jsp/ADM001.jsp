...
<center>
	<table class="tbl_input" cellpadding="4" cellspacing="0" width="400px">
		...
		<!-- Start vùng hiển thị message -->
		<c:if test="${lsErrMessage != null}">
			<c:forEach items="${lsErrMessage}" var="err" >
				<tr>
					<td class="errMsg" colspan="2">${err}</td>
				</tr>
			</c:forEach>
		</c:if>
		<!-- Start vùng hiển thị message -->
		...
		<tr>
			...
			<td align="left">
				<input class="txBox" type="password" name="password" value="${password}"	size="20" onfocus="this.style.borderColor='#0066ff';"
				onblur="this.style.borderColor='#aaaaaa';" />
			</td>
		</tr>
		<tr>
